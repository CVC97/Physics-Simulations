...
class lorenz_attractor_scene(ThreeDScene):
    def construct(self):
        ...
        traj1 = Dot(point = ax.c2p(x1_array[0], y1_array[0], z1_array[0]), radius = 0.08, color = WHITE).set_opacity(0.2)
        traj1.color = WHITE
        traj1.x_iter = x1_iter
        traj1.y_iter = y1_iter
        traj1.z_iter = z1_iter

        traj2 = Dot(point = ax.c2p(x2_array[0], y2_array[0], z2_array[0]), radius = 0.08, color = RED).set_opacity(0.2)
        traj2.color = RED
        traj2.x_iter = x2_iter
        traj2.y_iter = y2_iter
        traj2.z_iter = z2_iter

        traj3 = Dot(point = ax.c2p(x3_array[0], y3_array[0], z3_array[0]), radius = 0.08, color = BLUE).set_opacity(0.2)
        traj3.color = BLUE
